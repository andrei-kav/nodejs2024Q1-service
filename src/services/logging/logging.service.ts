...
export class LoggingService extends ConsoleLogger implements LoggerService {
    ...
    constructor(
        ...
        this.parseLogDirArg()
        this.logFileSize = Number(configService.get('LOG_FILE_SIZE')) || DEFAULT_LOG_FILE_SIZE
    ...
    private createFile(fileName: string) {
        let pathToFile = path.join(this.logDir, fileName)
        // try {
    ...
    private parseLogLevelArg() {
        const split = (this.configService.get('LOG_LEVEL')?.trim()
            .split(' ') as Array<LogLevel>)
        ...
        this.logLevel = [LogLevel.LOG, LogLevel.ERROR, LogLevel.WARN, LogLevel.DEBUG, LogLevel.VERBOSE]
    }
    ...
    private register(level: LogLevel, message: string) {
        if (this.needToRegister(level)) {
            super[level](message)
            const formatted = new Date().toISOString() + '_' + level.toUpperCase() + '_' + message + '\n'
